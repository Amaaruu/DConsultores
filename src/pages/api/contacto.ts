...
const success = { success: true, message: 'Recibimos los antecedentes de tu proyecto. Revisaremos la información y nos pondremos en contacto contigo.' };
...
export const POST: APIRoute = async ({ request, clientAddress }) => {
  ...
  const turnstileOk = await verifyTurnstile(payload.turnstileToken, clientAddress);
...
async function verifyTurnstile(token = '', remoteip?: string) {
...
async function sendContactEmail(payload: ProjectBriefPayload) {
  ...
  const text = [
    ...
    `Situacion actual: ${payload.currentSituation}`,
    ...
    `Migracion requerida: ${payload.migrationRequired}`,
    `Origen de migracion: ${payload.migrationSource || 'No indicado'}`,
